...
void ondemandwifiCallback () {
   ...
   if (!wifiManager.startConfigPortal("PzemV3_Em")) { Serial.println("Not connected to WiFi but continuing anyway.");} else { Serial.println("connected...yeey :)");}                
    ...
    value = wifiManager.server->arg(name);
      if (value == "1"){ 
        //PZEM004Tv30 pzem(5, 4);
         //pzem.resetEnergy();  // support is still missing
         Serial.println("clearStorage not suport");
         }
  }
  return value;
}
...
void setup() {
  ...
  if (SPIFFS.begin()) {  // ------------------------- wificonfig read -----------------
   ...
   char AUTHKEY[SUPLA_AUTHKEY_SIZE] = {mac[WL_MAC_ADDR_LENGTH - 1], mac[WL_MAC_ADDR_LENGTH - 2],mac[WL_MAC_ADDR_LENGTH - 3], mac[WL_MAC_ADDR_LENGTH - 4],
    ...
    new Supla::Sensor::PZEMv3(4, 5); // (RX,TX) connect to TX,RX of PZEM
